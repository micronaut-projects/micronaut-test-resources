...
You can disable application-side test resources resolution by setting `test-resources.enabled=false` in the application's configuration or passing `-Dtest-resources.enabled=false` to the application.
For example, configure the Gradle `run` task with `jvmArgs("-Dtest-resources.enabled=false")`, or start a packaged application with `java -Dtest-resources.enabled=false -jar application.jar`.
This prevents the application from resolving missing properties from a configured test resources server, including during local `run` task startup.
It does not disable build-tool behavior that may start or configure a test resources server before application startup.
