...
On the `4.0.x` line, the client/server RPC boundary uses a dedicated binary media type instead of JSON.
This keeps the remote protocol narrow and removes the JSON transport requirement from the client side.
