...
snippet::io.micronaut.testresources.docs.core.MyTest[project-base="test-suite-docs", tags="imports,clazz", languages="java,kotlin,groovy", title="Supplying test properties with a TestPropertyProvider"]

<1> The properties are added to the test properties before the application context is started

[.lang-java.lang-kotlin]
NOTE: With JUnit 5, a test class implementing `TestPropertyProvider` must use the `@TestInstance(TestInstance.Lifecycle.PER_CLASS)` lifecycle.

[.lang-python]
NOTE: A Python test class cannot implement `TestPropertyProvider` yet: Micronaut Test reads the test properties before the application context, and with it the Python runtime, is started. Supply such properties from a Java `ApplicationContextConfigurer` (`@ContextConfigurer`) instead.
...
snippet::io.micronaut.testresources.docs.core.ConnectionSpec[project-base="test-suite-docs", tags="imports,test", languages="java,kotlin,groovy", title="Computing test properties with @TestResourcesProperties"]

<1> The test resource properties which are needed to compute the new properties
<2> The provider which derives the new properties from the test properties
...
snippet::io.micronaut.testresources.docs.core.ConnectionSpec[project-base="test-suite-docs", tags="provider", languages="java,kotlin,groovy", title="Implementing a TestResourcesPropertyProvider"]

<1> The `rabbitmq.uri` property was resolved by test resources and is available in the test properties
<2> The computed `rabbitmq.servers.product-cluster.port` property is added to the test properties

[.lang-python]
NOTE: `@TestResourcesProperties` is not supported on Python test classes yet: the annotation is read from the test class, and the provider instantiated, before the application context, and with it the Python runtime, is started. Write the annotated test and its `TestResourcesPropertyProvider` in Java, Kotlin or Groovy.
