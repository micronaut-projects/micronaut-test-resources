...
snippet::io.micronaut.testresources.docs.junitplatform.ScopeNameTest[project-base="test-suite-docs", tags="imports,clazz", title="Declaring an explicit test resources scope"]

<1> All tests using the `my scope` scope share the same test resources
<2> The current scope is available to the test resources extension while the test runs
...
snippet::io.micronaut.testresources.docs.junitplatform.ScopeNamingStrategyTest[project-base="test-suite-docs", tags="imports,clazz", title="Declaring a test resources scope with a naming strategy"]

<1> The naming strategy computes the scope name from the test class
<2> With the `TestClassName` strategy, the scope is the fully qualified name of the test class
