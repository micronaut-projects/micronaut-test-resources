...
* Unless `test-resources.containers.redis.wait-strategy` is configured, the single-node Redis provider waits for the mapped Redis port to listen before exposing `redis.uri`.
