...
* The default image (`{default-image-redis}`, or `{default-image-redis-cluster}` in cluster mode) can be overwritten by setting the `test-resources.containers.redis.image-name` property.
