...
* Unless a Redis wait strategy is configured, for example with `test-resources.containers.redis.wait-strategy` or `test-resources.containers.redis.wait-strategy.*` properties, the single-node Redis provider waits for the mapped Redis port to listen before exposing `redis.uri`.
