...
For example, you may need a container image or protocol mapping that is not covered by a dedicated module.
...
snippet::io.micronaut.testresources.docs.testcontainers.JavamailSessionProvider[project-base="test-suite-docs", tags="imports,clazz", title="Using the properties of a generic container"]
...
Micronaut Test Resources uses the underlying container's default https://www.testcontainers.org/features/startup_and_waits/[Testcontainers wait strategies] unless you configure `test-resources.containers.<name>.wait-strategy...` or a provider documents a more specific default.

It is possible to override the default by configuration.
